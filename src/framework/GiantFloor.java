...
public class GiantFloor {
    ...
    public void draw(Program prog){
        prog.setUniform("worldMatrix", math3d.translation(mPosition));
    ...
    public void addAllFloors(Floor[] floors){
        for(int i = 0; i < floors.length; i++){
            this.addToVertData(floors[i].getVData());
            this.addToIndexData(floors[i].getIData(), i);
        }
    }
    ...
    public  void addToIndexData(int[] indicies, int offset){
        for(int i = 0; i < indicies.length; i++){
            mIData.add(indicies[i] + (4 * offset));
        }
    }
